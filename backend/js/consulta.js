...
function validarID(valID) {
    if (valID === null || Number.isNaN(valID) || valID < 1) {
        return false;
    }
...
function exibirErro(msg, fundoModal, modal) {
    alternarModal(fundoModal, modal);
    alternarModalMsg(msg);
}

function limparForm(id, form) {
    limparID(id);
    form.reset();
}
...
formEdicao.addEventListener("submit", function (e) {
    ...
    if (!validarID(valIDEdicao)) {
        exibirErro("Erro: O ID do produto é obrigatório e deve ser um número maior ou igual a 1.", fundoModalEdicao, modalEdicao);

        limparForm(idEdicao, this);
        return;
    }

    if (valNome === null || valNome === "" || valNome.length > 50) {
        exibirErro("Erro: O nome do produto é obrigatório e deve conter até 50 caracteres.", fundoModalEdicao, modalEdicao);
    ...
    if (!validarID(valTipo)) {
        exibirErro("Erro: Tipo de produto inválido.", fundoModalEdicao, modalEdicao);
        limparForm(idEdicao, this);
        return;
    }

    if (valPreco === null || Number.isNaN(valPreco) || valPreco < 1 || valPreco > 999.99) {
        exibirErro("Erro: O preço do produto é obrigatório e deve ser um número entre 1 e 999,99.", fundoModalEdicao, modalEdicao);
...
botaoCancelarEdicao.addEventListener("click", function () {
    alternarModal(fundoModalEdicao, modalEdicao);
    limparForm(idEdicao, formEdicao);
});
...
formDelecao.addEventListener("submit", function (e) {
    ...
    if (!validarID(valIDDelecao)) {
        exibirErro("Erro: O ID do produto é obrigatório e deve ser um número maior ou igual a 1.", fundoModalConfirmacao, modalConfirmacao);
